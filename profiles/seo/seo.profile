 ...
 *   task list.
 */
function seo_profile_task_list() {
  global $conf;
 ...
 */
function seo_profile_tasks(&$task, $url) {
  global $base_url, $install_locale, $language;
  // Just in case some of the future tasks adds some output
  $output = '';

  if ($task == 'profile') {
    _seoprofile_modify_settings();
    _seoprofile_set_content_types();
    _seoprofile_modify_blocks();
  ...
  if ($task == 'optional-modules') {
    $output = drupal_get_form('seo_optional_modules_form', $url);
    ...
    if (!variable_get('ipkg_modules_install', FALSE)) {
      ...
      if (count($modules_list) > 0) {
        ...
        $operations = profile_download_files('seo', 'optional_modules', array_keys($modules_list), $ignore, 6);

        $batch = array(
          'operations' => $operations,
          'finished' => '_install_seo_theme_batch',
  ...
  if ($task == 'optional-themes') {
    $output = drupal_get_form('seo_optional_themes_form', $url);
    ...
    if (!variable_get('ipkg_themes_install', FALSE)) {
      ...
      if (count($files_list) > 0) {
        ...
        $operations = profile_download_files('seo', 'themes', array_keys($files_list), $ignore, 6);

        $batch = array(
          'operations' => $operations,
          'finished' => '_install_seo_theme_batch_finished',
 ...
 */
function _install_seo_theme_batch($success, $results) {
  variable_del('ipkg_modules_install');
 ...
 */
function _install_seo_theme_batch_finished($success, $results) {
  variable_del('ipkg_themes_install');
...
/**
 * Form API submit for the seo_optional_modules form.
 *
 * Handle optional modules.
 */
function seo_optional_modules_form_submit($form, &$form_state) {
...
/**
 * Form API submit for the seo_optional_themes form.
 *
 * Handle optional themes.
 */
function seo_optional_themes_form_submit($form, &$form_state) {
 ...
 */
function _seoprofile_modify_settings() {
  global $theme_key;
 ...
 */
function _seoprofile_modify_blocks() {
 ...
 */
function seo_form_alter(&$form, $form_state, $form_id) {
